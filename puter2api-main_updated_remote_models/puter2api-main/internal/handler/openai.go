...
func (h *Handler) HandleModels(c *gin.Context) {
	...
	// 1) If MODELS_SOURCE=remote, fetch Puter models list (cached) first.
	// 2) Fall back to local model.json (ships with the repo).
	// 3) Finally, fall back to the original hardcoded list.
	var (
		ids []string
		err error
	)
	if modelsSourceRemoteEnabled() {
		ids, err = loadModelIDsRemoteCached(c.Request.Context())
	}
